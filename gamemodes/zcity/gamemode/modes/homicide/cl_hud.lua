	...
	if(ply:Alive())then
		if(ply.isTraitor)then
			if(ply.SubRole == "traitor_infiltrator" or ply.SubRole == "traitor_infiltrator_soe")then
				...
				if(IsValid(aim_ent))then
					if(aim_ent:IsRagdoll())then
						local text = "[ALT + R] Сменить внешность"
						local tw, th = surface.GetTextSize(text)
						local cx, cy = trace.HitPos:ToScreen().x, trace.HitPos:ToScreen().y
			...
			if(ply.SubRole == "traitor_assasin" or ply.SubRole == "traitor_assasin_soe" or ply.PlayerClassName == "sc_infiltrator")then
				...
				local text = "(HOLD)[ALT + E] Схватить за писюн"
				local tw, th = surface.GetTextSize(text)
				local cx, cy = trace.HitPos:ToScreen().x, trace.HitPos:ToScreen().y
				cy = cy + y_offset
...
hook.Add("HUDPaint", "DrawTraitorPanel", function()
    ...
    local title = is_main and "Предатель" or "Помощник предателя"
    ...
    draw.SimpleText("Нажми F4 чтобы открыть слова", "TraitorPanelText", x + traitor_panel.width/2, y + 42, 
                    Color(180, 180, 180, 220), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    
    local word_y = y + 65
    draw.SimpleText("Секретные слова:", "TraitorPanelText", x + traitor_panel.width/2, word_y, 
    ...
    if is_main then
        ...
        if has_assistants then
            draw.SimpleText("Твои помощники:", "TraitorPanelText", x + traitor_panel.width/2, assist_y, 
                            Color(220, 220, 220), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            ...
            draw.SimpleText("Помощники недоступны", "TraitorPanelText", x + traitor_panel.width/2, assist_y, 
